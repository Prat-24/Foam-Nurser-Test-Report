document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("saveAsPdf").addEventListener("click", function(event) {
        ...
        const row = {
            ...
            Shift: document.querySelector('input[name="shift"]').value,
            ...
            Ammeter: document.querySelector('select[name="Ammeter"]').value,
            RadiatorWater: document.querySelector('select[name="Radiatorwater"]').value,
            ValveLinkages: document.querySelector('select[name="ValveLinkages"]').value,
            TyreCondition: document.querySelector('select[name="Tyrecondition"]').value,
            VesselCondition: document.querySelector('select[name="Vesselcondition"]').value,
            DrainValveClosed: document.querySelector('select[name="DrainValveClosed"]').value,
            PressureOutletClosed: document.querySelector('select[name="PressureOutletClosed"]').value,
            TankSuctionPipeClosed: document.querySelector('select[name="TankSuctionPipeClosed"]').value,
            WiperCondition: document.querySelector('select[name="Wipercondition"]').value,
        
            // Static Run / Tank to Pump Operation
            CirculationJRCP: document.querySelector('input[name="Circulation/JRCP"]').value,
            StartTime: document.querySelector('input[name="starttime"]').value,
            StopTime: document.querySelector('input[name="stoptime"]').value,
            PTOEngagement: document.querySelector('select[name="PTO engagement"]').value,
            HandBrakes: document.querySelector('select[name="Handbrakes"]').value,
            TankToPumpValve: document.querySelector('select[name="TanktoPump"]').value,
            ...
            RecycleValve: document.querySelector('select[name="RecycleValve"]').value,
           ...
           // Observations: document.querySelector('textarea[name="Observations"]').value,
            //Maintenance: document.querySelector('textarea[name="Maintenance"]').value,
      ...
      pdf.setFontSize(12);
      pdf.text('Testing Of Nurser Truck', 95, subtitleY + 2, { align: 'center' });
  ...
  pdf.text('Doc No: HPCL MR/FRM/F&S/08200-04', textX, textY + 20);
...
if (label2) {
    const xLabel2Center = 105; // Adjust to place second label box directly after first value box
    ...
    // Define label text position with vertical adjustment for centering
// Define label1 properties

const label2FontSize = 11; // Font size for label2 (original size)
const textVerticalOffset = 2; // Adjust this based on font style and size
...
return positionY + 12; // Move down for the next row and return updated positionY
  ...
  for (const row of data) 
  {
    let positionY = addHeader(); // Add header for the page and get the new position
                    ...
                    // Initialize positionY and assign form data
positionY = 60; // starting y-position, can be adjusted

positionY = fieldPair('Date:', row.Date ?? '', 'Shift:', row.Shift ?? '', positionY);
positionY += 20; // Additional gap

// Vehicle Check Fields
positionY = fieldPair('Body Condition:', row.BodyCondition ?? '', 'Air Pressure:', row.AirPressure ?? '', positionY);
positionY = fieldPair('Engine Oil:', row.EngineOil ?? '', 'Ammeter:', row.Ammeter ?? '', positionY);
positionY = fieldPair('Radiator Water:', row.RadiatorWater ?? '', 'Valve Linkages:', row.ValveLinkages ?? '', positionY);
positionY = fieldPair('Tyre Condition:', row.TyreCondition ?? '', 'Vessel Condition:', row.VesselCondition ?? '', positionY);
positionY = fieldPair('Drain Valve Closed:', row.DrainValveClosed ?? '', 'Pressure Outlet Closed:', row.PressureOutletClosed ?? '', positionY);
positionY = fieldPair('Tank Suction Pipe:', row.TankSuctionPipeClosed ?? '', 'Wiper Condition:', row.WiperCondition ?? '', positionY);

// Additional fields can continue in the same format, increasing positionY as needed
positionY += 20; // Extra space before the next section if needed

                const y1 = 190;
                    const STATICRUNTANKTOPUMPOPERATIONText = '2.STATIC RUN /TANK TO PUMP OPERATION';
                    // Add the text outside the box, above it
                    pdf.text(STATICRUNTANKTOPUMPOPERATIONText, xText, y1 - 20);
// Initialize positionY and assign form data
positionY = fieldPair('Circulation/JRCP:', row.CirculationJRCP ?? '', '', '', positionY); // No second label for 'Circulation/JRCP'

positionY += 5; // Additional gap

positionY = fieldPair('Start Time:', row.StartTime ?? '', 'Stop Time:', row.StopTime ?? '', positionY);
positionY = fieldPair('PTO Engagement:', row.PTOEngagement ?? '', 'Hand Brakes working:', row.HandBrakes ?? '', positionY);
positionY = fieldPair('Tank to Pump Valve:', row.TankToPumpValve ?? '', 'Delivery Outlet & valve:', row.DeliveryOutlet ?? '', positionY);
positionY = fieldPair('Discharge Pressure:', row.DischargePressure ?? '', 'Engine RPM:', row.EngineRPM ?? '', positionY);
positionY = fieldPair('Foam Tank Level Before:', row.BeforeOperation ?? '', 'Foam Tank Level After:', row.AfterOperation ?? '', positionY);
positionY = fieldPair('Flushing Done:', row.FlushingDone ?? '', 'Recycle Valve Condition:', row.RecycleValve ?? '', positionY);

// Adjust positionY as needed for additional spacing
positionY += 20; // Extra space before the next section if needed
}
...
const remarksY = 80; // Position below the Maintenance section
...
const remarksTextareaY = 70;
const remarksTextareaWidth = 190; // Adjust the width as needed
...
/* Add the "Remarks" text
const ObservationsText = '4.OTHER OBSERVATIONS';
const ObservationsX = 10; // Adjust x position as needed
const ObservationsY = 130; // Position below the Maintenance section
pdf.text(ObservationsText, ObservationsX, ObservationsY - 20); // Adjust the y offset as needed

// Create the textarea for Remarks
const ObservationsTextareaX = 8.5;
const ObservationsTextareaY = 120;
const ObservationsTextareaWidth = 190; // Adjust the width as needed
...
const MaintenanceText = '5.MAINTENANCE FOLLOW UP REQUIRED';
const MaintenanceX = 10; // Adjust x position as needed
const MaintenanceY = 180; // Position below the Maintenance section
pdf.text(MaintenanceText, MaintenanceX, MaintenanceY - 20); // Adjust the y offset as needed

// Create the textarea for Remarks
const MaintenanceTextareaX = 8.5;
const MaintenanceTextareaY = 170;
const MaintenanceTextareaWidth = 190; // Adjust the width as needed
...
for (let i = 1; i <= totalPages; i++) {
    ...
    pdf.text(`Page ${i} of ${totalPages}`, pdf.internal.pageSize.width / 2, pdf.internal.pageSize.height - 10, { align: 'center' });
      ...
      pdf.save("Foam_Nurser_Test_Report.pdf");
  });
});
